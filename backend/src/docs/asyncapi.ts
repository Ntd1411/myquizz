...
const description = [
  ...
  '## Errors',
  '',
  'Every failed handler answers with the same string, `CODE: sentence`. When the',
  'client sent an acknowledgement callback the message arrives as `{ error }` in',
  'that callback, otherwise it arrives as an `error` event. `UNAUTHORIZED`,',
  '`FORBIDDEN` and `GONE` are fatal: reconnecting with the same token will fail',
