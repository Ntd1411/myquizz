...
import ms from "ms"
import { generateTokens, hashPassword, verifyPassword, verifyToken, hashToken } from "../../shared/utils/auth.utils.js"
...
export async function loginService(email: string, password: string, deviceName: string, ipAddress: string) {
  // Find user by email
...
export async function refreshTokenService(refreshToken: string) {
  ...
  const refreshSession = await authRepository.findRefreshToken(hashedRefreshToken)
...
export async function logoutService(userId: number, accessToken: string, refreshToken: string) {
  const decodedRefreshToken = verifyToken(refreshToken, "refresh")
  const decodedAccessToken = verifyToken(accessToken, "access")

  if (!decodedAccessToken || decodedAccessToken.type !== "access" || decodedAccessToken.userId !== userId) {
  ...
  if (!decodedRefreshToken || decodedRefreshToken.type !== "refresh" || decodedRefreshToken.userId !== userId) {
    await authRepository.revokeUserSessions(userId)
    throw new AppError(401, "Invalid refresh token")
  }
